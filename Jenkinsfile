pipeline {
  ...
  environment {
    APP_NAME='FlaskApp'
    CONTACT_EMAIL='munitmahato@gmail.com'
  }

  stages {
    stage('Checkout') {
      steps {
         git branch: 'main', url: 'https://github.com/Munit4596/FlaskTest.git'
      }
    }
    ...
    stage('Deploy (staging)') {
      steps {
        sh '''
          ...
          echo $! > flask.pid
          sleep 10
          curl -f http://3.65.85.85:8080/ || (echo "Smoke test failed"; cat flask.log; exit 1)
        '''
      }
    }
  }
